...
async function getDashboardContext(supabase: Awaited<ReturnType<typeof createClient>>) {
  ...
  const [settingsRes, leadsRes, tasksRes, kpisRes] = await Promise.all([
    ...
    sb
      .from("kpis")
      .select("metric_name, metric_value, metric_target, unit, period_month, category")
  ...
  const taskLines = [
    ...
    urgentTasks.length > 0
      ? `  - URGENTES: ${urgentTasks.map((t: any) => `"${t.title}"`).join(", ")}`
      : null,
    inProgress.length > 0
      ? `  - En progreso (${inProgress.length}): ${inProgress.slice(0, 3).map((t: any) => `"${t.title}"`).join(", ")}`
      : null,
...
async function checkCredits(sb: any): Promise<{ ok: boolean; used: number; limit: number }> {
  const { data } = await sb
...
async function incrementCredits(tokensUsed: number): Promise<void> {
  try {
    ...
    console.error("[ai/chat] incrementCredits error:", err)
  }
}
...
export async function POST(req: NextRequest) {
  ...
  // ── Credits check — block before touching Anthropic ───────────────────────
  const credits = await checkCredits(sb)
  if (!credits.ok) {
    return Response.json(
      {
        error:   "credits_exceeded",
        message: "Llegaste al limite de creditos de IA para este periodo. Contacta al admin para aumentarlo.",
        used:    credits.used,
        limit:   credits.limit,
      },
      { status: 429 },
...
INSTRUCCIONES:
- Responde siempre en espanol rioplatense. Usa "vos", "tenes", "podes".
- Se directo y accionable. Evita intro largas.
- Cuando des recomendaciones, basate en los datos reales del contexto.
- Usa **negrita** para resaltar, listas con - cuando haya multiples items.
- Si no tenes el dato exacto, decilo claramente pero ayuda igual.
- Actua como un consultor senior que conoce la empresa a fondo.
- Fecha de hoy: ${today}`
  ...
  const readable = new ReadableStream({
    async start(controller) {
      try {
        ...
        // Increment credit counter (service role, bypasses RLS)
        if (tokensUsed > 0) {
          incrementCredits(tokensUsed).catch(err =>
            console.error("[ai/chat] Failed to increment credits:", err)
          )
        }
      }
    },
